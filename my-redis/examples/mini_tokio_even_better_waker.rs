...
// The snippet above creates a Delay instance, polls it once, 
// then sends the Delay instance to a new task where it is awaited. 
// In this example, Delay::poll is called more than once (twice here) with different Waker instances.
// Our earlier implementation did not handle this case and the spawned task would sleep forever, as the wrong task is notified.
